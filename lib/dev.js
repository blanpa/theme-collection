;(async () => {
    ...
    const customCSSFile = path.join(themeSourceDir, 'theme-custom.css')
    ...
    const commonCssFile = path.join(rootDir, 'src/common/common.css')
    ...
    watch([sassFile, customCSSFile, mermaidThemeFile, monacoThemeFile, commonCssFile]).on('change', () => {
        buildTheme(themeName)
    })

    nodemon({
        exec: `node "${rootDir}/node-red/packages/node_modules/node-red/red.js" \
            --port 41880 \
            --define uiHost=0.0.0.0 \
            --userDir .node-red \
